<?php
    ...
    class IniReader
    {
         ...
         * @throws ConfigurationException
         */
        public function readString(string $ini)
        {
         ...
         * @throws ConfigurationException
         */
        private function readWithNativeFunction(string $ini)
        {
            $array = @parse_ini_string($ini, true);

            if ($array === false) {
                $e = error_get_last();
                throw new ConfigurationException('Syntax error in INI configuration: ' . $e['message']);
            }
        ...
        private function getContentOfIniFile($filename)
        {
            if (!file_exists($filename) || !is_readable($filename)) {
                throw new ConfigurationException(sprintf("The file %s doesn't exist or is not readable", $filename));
            }

            $content = $this->getFileContent($filename);

            if ($content === false) {
                throw new ConfigurationException(sprintf('Impossible to read the file %s', $filename));
            }
         ...
         * @throws ConfigurationException
         */
        public function readComments(string $filename): array
        {
            $ini = $this->getContentOfIniFile($filename);
         ...
         *
         */
        private function readWithAlternativeImplementation(string $ini)
        {
            $ini = $this->splitIniContentIntoLines($ini);
